...
export const CarCard: FC<CarCardProps> = ({car, gridMode}) => {
    return (
        <div className={`flex ${gridMode ? 'flex-col rounded-lg shadow-md max-w-[296px]' : 'flex-row items-center'} ${!gridMode ? 'border-b-2 border-gray-300/50' : ''} ${gridMode ? 'bg-white' : ''} gap-4 p-2`}>
            <div className={`relative ${!gridMode ? 'h-[106px] w-[120px]' : ''} md:max-w-[280px] md:max-h-[196px] overflow-hidden rounded-lg`}>
                <Button isIconOnly variant="solid" radius="full" size="sm" className={`absolute top-1 right-1 z-50 ${gridMode ? 'bg-white' : 'bg-white/50'}`}>
                    <BsHeart />
                </Button>
                <img 
            ...
            <div className="flex flex-col gap-1">
                <div className="flex gap-2 items-center">
                    <Chip variant="flat" size="sm">{car.year}</Chip>
                    <Chip variant="flat" size="sm"><NumberFormatter value={car.mileage} endContent="km"/></Chip>
                </div>
                <div className={!gridMode ? `text-sm gap-1` : `text-sm md:text-md`}>
                    {
                        ...
                        :
                        <p className="font-semibold capitalize">{car.model}{" "}<span className="font-normal">{car.version}</span></p>
                    }
                </div>
            ...
            {gridMode && 
                <div className="grid place-items-center">
                    <Button className="text-sm font-semibold bg-blue-800 text-white" fullWidth startContent={<CashRegisterIcon/>}>
                        Simular parcelas
                    </Button>
                </div>
            }
